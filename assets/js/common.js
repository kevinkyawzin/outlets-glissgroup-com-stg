    ...
    function getDateFromAspString(aspString) {
        ...
        '$1');
        if (epochMilliseconds != aspString) {
            //var retDate = new Date(new Date(parseInt(epochMilliseconds)).getTime() + offset * 3600 * 1000).toUTCString().replace(/ GMT$/, "")
            //console.log(retDate);

            // hour adjustment// LIVE server timezone is US (-8) we need (+8) timezone for SG, HK

            var dbDate = new Date(parseInt(epochMilliseconds));

            var offset = dbDate.getTimezoneOffset();
        //    console.log("TimeZoneOffset : " + offset)

            var milliseconds = (offset - 480) * 60 * 1000;


            // adjust time for US time zone
            //var returnDate = new Date(parseInt(epochMilliseconds) + milliseconds);

            var returnDate = new Date(parseInt(epochMilliseconds));
    ...
    function getReportHeader(branchcode) {
        ...
        switch (branchcode) {
            case 'GWS':
                content += '	<div style="text-align:center;"> Gliss Wellness and Spa Pte. Ltd.</div>	'
